...
async def test_tv_enrichment_success(async_session: AsyncSession):
    ...
    responses = {
        "/search/tv": {"status": 200, "data": {"results": [{"id": 136315, "name": "The Bear", "first_air_date": "2022-06-23"}]}},
        "/tv/136315/season/1/episode/1": {"status": 200, "data": {"name": "System", "episode_number": 1, "overview": "Carmy returns to Chicago."}}
    }
...
async def test_tv_enrichment_series_not_found(async_session: AsyncSession):
    ...
    await async_session.commit()
    responses = {
        "/search/tv": {"status": 200, "data": {"results": []}}
    }
    client = httpx.AsyncClient(transport=MockTransport(responses))
    service = TVMetadataService(api_key="dummy", client=client)
...
async def test_tv_enrichment_caching(async_session: AsyncSession):
    ...
    responses = {
        "/search/tv": {"status": 200, "data": {"results": [{"id": 136315, "name": "The Bear", "first_air_date": "2022-06-23"}]}},
        "/tv/136315/season/1/episode/1": {"status": 200, "data": {"name": "System", "episode_number": 1, "overview": "Carmy returns to Chicago."}},
        "/tv/136315/season/1/episode/2": {"status": 200, "data": {"name": "Hands", "episode_number": 2, "overview": "Chaos in the kitchen."}}
    }
    client = httpx.AsyncClient(transport=MockTransport(responses))
    service = TVMetadataService(api_key="dummy", client=client)
