...
def step_then_output_format(context, format):
    if not context.output_path.endswith(f".{format}"):
        raise AssertionError(
            f"Expected output format: {format}, but got: {context.output_path.split('.')[-1]}"
        )


@then('the bitrate should be "{bitrate}"')
def step_then_bitrate(context, bitrate):
    # Placeholder for bitrate validation
    context.metadata["bitrate"] = bitrate


@then('the sample rate should be "{sample_rate}"')
...
@given('the metadata includes "Artist: {artist}" and "Title: {title}"')
...
def step_when_audio_normalized(context):
    ...
    context.output_path = (
        f"{context.metadata['artist']} - {context.metadata['title']}.mp3"
    )


@then('the file name should be "{expected_file_name}"')
def step_then_file_name(context, expected_file_name):
    if context.output_path != expected_file_name:
        raise AssertionError(
            f"Expected file name: {expected_file_name}, but got: {context.output_path}"
        )


@then("the file name should align with Music Assistant standards")
...
@given('the metadata includes "Artist: {artist}", "Album: {album}", and "Year: {year}"')
...
@then('the file should be moved to "{expected_path}"')
def step_then_file_moved(context, expected_path):
    if context.output_path != expected_path:
        raise AssertionError(
            f"Expected path: {expected_path}, but got: {context.output_path}"
        )


@then("the directory structure should align with Music Assistant standards")
...
@then("the metadata should include artist, album, title, and album art")
def step_validate_flac_metadata(context):
    ...
    for key, value in expected_metadata.items():
        if context.metadata.get(key) != value:
            raise AssertionError(
                f"Metadata mismatch for {key}: expected {value}, got {context.metadata.get(key)}"
            )
