...
@given('a TV show file "{file_name}"')
...
def step_then_metadata_title(context, title):
    if context.metadata.get("title") != title:
        raise AssertionError(
            f"Expected title: {title}, but got: {context.metadata.get('title')}"
        )


@then('the metadata should include "Season: {season}"')
def step_then_metadata_season(context, season):
    if context.metadata.get("season") != int(season):
        raise AssertionError(
            f"Expected season: {season}, but got: {context.metadata.get('season')}"
        )


@then('the metadata should include "Episode: {episode}"')
def step_then_metadata_episode(context, episode):
    if context.metadata.get("episode") != int(episode):
        raise AssertionError(
            f"Expected episode: {episode}, but got: {context.metadata.get('episode')}"
        )
