...
def step_then_resolution(context, resolution):
    if context.metadata.get("resolution") != resolution:
        raise AssertionError(
            f"Expected resolution: {resolution}, but got: {context.metadata.get('resolution')}"
        )


@then('the codec should be "{codec}"')
def step_then_codec(context, codec):
    if context.metadata.get("codec") != codec:
        raise AssertionError(
            f"Expected codec: {codec}, but got: {context.metadata.get('codec')}"
        )
