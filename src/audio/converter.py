...
class AudioConverter:
    ...
    def __init__(
        self, output_format: str = "flac", sample_rate: int = 44100, bit_depth: int = 16
    ):
    ...
    async def convert(self, input_file: Path, output_dir: Path) -> Optional[Path]:
        ...
        print(
            f"Converting {input_file} to {output_file} with sample rate {self.sample_rate} Hz and bit depth {self.bit_depth}..."
        )
